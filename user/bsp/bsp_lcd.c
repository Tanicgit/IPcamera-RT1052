...
SemaphoreHandle_t myCountingSemLcdHandle;

/* 帧数计数器，使能帧中断才有效 */
__IO uint32_t s_frame_count = 0;

volatile uint8_t s_now_buffer = 0;
/* 显存 */
AT_SDRAM1_SECTION_ALIGN(pixel_t s_psBufferLcd[2][LCD_PIXEL_HEIGHT][LCD_PIXEL_WIDTH],
                              FRAME_BUFFER_ALIGN);
...
* @retval 无
*/
void LCD_Board_Init(bool enableInterrupt)
{
#if LCD_RGB_888	
	/* 
  ...
  *((uint32_t *)0x41044100) = 0x0000000f;
  ...
  LCD_BackLight_ON();
	...
	myCountingSemLcdHandle = xSemaphoreCreateCounting( 2, 0);
...
摄像头和LCD像素不一定匹配
*/
void LCD_SetNextBufferAddr(camera_config_t *config,uint32_t addr)
{
	uint16_t i,j;
	uint8_t *data;
	s_now_buffer ^= 1;
	switch(lcdConfig.pixelFormat)
	{
		case kELCDIF_PixelFormatRGB565:
			if(config->pixelFormat ==  kVIDEO_PixelFormatRGB565 )
			{
				for(i=0;i<lcdConfig.panelHeight && i< (config->resolution >>16) ;i++)
				{
					if((config->resolution&0xffff) <= lcdConfig.panelWidth)
					{
						memcpy(s_psBufferLcd[s_now_buffer][i],(uint8_t*)(addr + i*config->frameBufferLinePitch_Bytes),config->frameBufferLinePitch_Bytes);
					}
					else
					{
						memcpy(s_psBufferLcd[s_now_buffer][i],(uint8_t*)(addr+i*lcdConfig.panelWidth*2),lcdConfig.panelWidth*2);
					}
				}
//				for(i=0;i<240;i++)
//				{
//					for(j=0;j<320;j++)
//					{
//						data = (uint8_t *)(addr+i*320+j);
//						s_psBufferLcd[s_now_buffer][2*i][2*j] = *data;
//						s_psBufferLcd[s_now_buffer][2*i][2*j+1] = *data;
//						
//						s_psBufferLcd[s_now_buffer][2*i+1][2*j] = *data;
//						s_psBufferLcd[s_now_buffer][2*i+1][2*j+1] = *data;
//					}				
//				}
	...
	ELCDIF_SetNextBufferAddr(LCDIF, (uint32_t)s_psBufferLcd[s_now_buffer]);	
	CurrentFrameBuffer = (uint32_t)s_psBufferLcd[s_now_buffer];
}

void wait_lcd_frame_isr()
{
	//等待帧中断
	if( xSemaphoreTake( myCountingSemLcdHandle, portMAX_DELAY ) == pdTRUE )
	{
	} 
	else
	{
   ...
   if (intStatus & kELCDIF_CurFrameDone)
    {
			xHigherPriorityTaskWoken = pdFALSE;
				...
				xSemaphoreGiveFromISR( myCountingSemLcdHandle, &xHigherPriorityTaskWoken );
				/* 帧计数器 */
				s_frame_count++;
