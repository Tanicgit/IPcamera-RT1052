...
#include <stdlib.h>
#include "bsp.h"
#include "lwip_app.h"       // BSD socket interface
...
*/
static void vTaskCodeVNC(void * argument) {
  int s, Sock;
	uint32_t AddrLen;
  U16 Port;
  ...
  while (1) {
    ...
    if ((Sock = accept(s, (struct sockaddr*)&_Addr, &AddrLen)) == -1) {
      continue; // Error
		...
		char*p = inet_ntoa(_Addr.sin_addr);
		Ac_log("%s connect\r\n",p);
    //
...
*/
BaseType_t xReturnedVNC;
TaskHandle_t xHandleVNC = NULL;
int GUI_VNC_X_StartServer(int LayerIndex, int ServerIndex) {
  ...
  //
	xReturnedVNC = xTaskCreate(vTaskCodeVNC, "_ServerVNC",512, ( void * ) 1,9,&xHandleVNC );
	if(xReturnedVNC==-1)
	{
		__aeabi_assert("VNC Creat err",__FILE__,__LINE__);
	}
